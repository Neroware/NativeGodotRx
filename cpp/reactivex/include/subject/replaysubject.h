...
public:
    ...
    RemovableDisposable(const rx_subject_t& subject_, const std::shared_ptr<Observer>& observer_) 
...
protected:
    ReplaySubject(uint64_t buffer_size_ = 1024ul, const time_delta_t& window_ = std::chrono::milliseconds::max(), const scheduler_t& scheduler_ = CurrentThreadScheduler::singleton())
        : buffer_size(buffer_size_), window(window_), scheduler(scheduler_) {}
public:
    ~ReplaySubject(){ this->dispose(); }
    inline static std::shared_ptr<ReplaySubject> get(uint64_t buffer_size_ = 1024ul, const time_delta_t& window_ = std::chrono::milliseconds::max(), const scheduler_t& scheduler_ = CurrentThreadScheduler::singleton()) { return std::shared_ptr<ReplaySubject>(new ReplaySubject(buffer_size_, window_, scheduler_)); }
