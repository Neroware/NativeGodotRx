...
public:
    std::shared_ptr<DisposableBase> underlying_disposable;
...
protected:
    RefCountDisposable(const std::shared_ptr<DisposableBase>& disposable) 
        : underlying_disposable(disposable) {}
public:
    ~RefCountDisposable(){ this->dispose(); }
    inline static std::shared_ptr<RefCountDisposable> get(const std::shared_ptr<DisposableBase>& disposable_) {
