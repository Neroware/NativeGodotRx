...
public:
    ...
    void on_error(const error_t& error) override;
    void on_completed() override;

private:
    virtual void _on_next_core(const Variant& item);
    virtual void _on_error_core(const error_t& error);
    virtual void _on_completed_core();

public:
    virtual void dispose() override;

    bool fail(const error_t& error);
    [[noreturn]] void throw_error(const error_t& error) const;
    
    notifier_t to_notifier();
    observer_t as_observer();
