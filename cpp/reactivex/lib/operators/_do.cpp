...
observable_op_t ops::do_action_(
    const on_next_t& on_next,
    const on_error_t& on_error,
    const on_completed_t& on_completed
) {
...
observable_op_t ops::do_(const observer_t& observer) {
    return do_action_(ONNEXT_FWD(observer), ONERROR_FWD(observer), ONCOMPLETED_FWD(observer));
}

rx_observable_t ops::do_after_next(const rx_observable_t& source, const on_next_t& after_next) {
...
rx_observable_t ops::do_on_subscribe(const rx_observable_t& source, const action_t& on_subscribe) {
    auto subscribe = SUBSCRIBE(observer, scheduler = nullptr) {
        on_subscribe();
        return source->subscribe(
...
rx_observable_t ops::do_on_dispose(const rx_observable_t& source, const dispose_t& on_dispose) {
...
rx_observable_t ops::do_on_terminate(const rx_observable_t& source, const action_t& on_terminate) {
    auto subscribe = SUBSCRIBE(observer, scheduler = nullptr) {
...
rx_observable_t ops::do_after_terminate(const rx_observable_t& source, const action_t& after_terminate) {
...
observable_op_t ops::do_finally_(const action_t& finally_action) {

    class OnDispose : public DisposableBase {
    public:
        OnDispose(const action_t& finally_action, const std::shared_ptr<bool>& was_invoked) 
