...
observable_op_t ops::group_by_until_(
    const mapper_t<Variant, Variant>& key_mapper,
    const mapper_t<rx_observable_t, rx_grouped_observable_t>& duration_mapper,
        ...
        subscription_t subscribe = SUBSCRIBE(observer, scheduler = nullptr) {
            ...
            on_next_t on_next = [=](const Variant& x) {
                ...
                if (fire_new_map_entry) {
                    ...
                    sad->set_disposable(duration->pipe(take(1))->subscribe(
                        on_next, on_error, 
