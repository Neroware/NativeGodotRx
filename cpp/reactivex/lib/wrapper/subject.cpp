...
Ref<RxDisposable> RxSubject::subscribe(const Variant **args, GDExtensionInt arg_count, GDExtensionCallError &error) {
    Variant _args[4];
    for (auto i = 0; i < 4; i++) {
...
void RxSubject::on_next(const Variant& item) {
    this->_ptr->on_next(item);
}

void RxSubject::on_error(Ref<RxError> error) {
    this->_ptr->on_error(RxError::unwrap(error));
}

void RxSubject::on_completed() {
