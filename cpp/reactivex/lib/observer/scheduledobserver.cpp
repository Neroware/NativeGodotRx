...
void ScheduledObserver::ensure_active() {
    ...
    if (is_owner) {
        auto self = getptr();
        action_t run_ = [self](const std::shared_ptr<SchedulerBase>& scheduler_, const Variant& state_) { 
...
void ScheduledObserver::run(const std::shared_ptr<SchedulerBase>& scheduler, const Variant& state) {
    auto parent = getptr();
