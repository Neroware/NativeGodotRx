...
static time_point_t _get_duetime(const time_point_t& t, const scheduler_t& s) {
    return t;
}
static time_point_t _get_duetime(const time_delta_t& t, const scheduler_t& s) {
    return s->now() + t;
}
static time_point_t _get_duetime(const double t, const scheduler_t& s) {
    return s->now() + Scheduler::to_timedelta(t);
}

static rx_observable_t observable_timer_date(const time_point_t& duetime, const scheduler_t& scheduler = nullptr) {
...
static rx_observable_t observable_timer_duetime_and_period(const timeT0& duetime_, const timeT1& period, const scheduler_t& scheduler = nullptr) {
...
static rx_observable_t observable_timer_timespan(const time_delta_t& duetime, const scheduler_t& scheduler = nullptr) {
    subscription_t subscribe = SUBSCRIBE(observer, scheduler_ = nullptr) {
        auto _scheduler = scheduler ? scheduler : scheduler_ ? scheduler_ : SceneTreeTimeoutScheduler::singleton();
...
static rx_observable_t observable_timer_timespan_and_period(const time_delta_t& duetime, const time_delta_t& period, const scheduler_t& scheduler = nullptr) {
...
rx_observable_t obs::timer_(const time_point_t& duetime, const scheduler_t& scheduler) {
    return observable_timer_date(duetime, scheduler);
}
rx_observable_t obs::timer_(const time_point_t& duetime, const time_delta_t& period, const scheduler_t& scheduler) {
    return observable_timer_duetime_and_period(duetime, period, scheduler);
}
rx_observable_t obs::timer_(const time_delta_t& duetime, const scheduler_t& scheduler) {
    return observable_timer_timespan(duetime, scheduler);
}
rx_observable_t obs::timer_(const time_delta_t& duetime, const time_delta_t& period, const scheduler_t& scheduler) {
    return observable_timer_timespan_and_period(duetime, period, scheduler);
}
rx_observable_t obs::timer_(const double& duetime, const scheduler_t& scheduler) {
    return observable_timer_timespan(Scheduler::to_timedelta(duetime), scheduler);
}
rx_observable_t obs::timer_(const double& duetime, const time_delta_t& period, const scheduler_t& scheduler) {
